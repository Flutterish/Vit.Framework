...
public class Pipeline : DisposableVulkanObject<VkPipeline> {
	...
	public unsafe Pipeline ( VkDevice device, PipelineArgs args ) {
		...
		fixed ( VkDynamicState* dynamicStatesPtr = dynamicStates ) {
			...
			fixed ( VkVertexInputAttributeDescription* attribsPtr = attribs ) {
				fixed ( VkVertexInputBindingDescription* setsPtr = sets ) {
					var vert = new VkPipelineVertexInputStateCreateInfo() {
						...
						pVertexBindingDescriptions = setsPtr
					};
					...
					fixed ( VkDescriptorSetLayout* uniformsPtr = uniforms ) {
						var layoutInfo = new VkPipelineLayoutCreateInfo() {
							sType = VkStructureType.PipelineLayoutCreateInfo,
							setLayoutCount = (uint)uniforms.Length,
							pSetLayouts = uniformsPtr
						};

						Vk.vkCreatePipelineLayout( Device, &layoutInfo, VulkanExtensions.TODO_Allocator, out Layout ).Validate();

						Span<GCHandle> entryPointPins = stackalloc GCHandle[shaders.Modules.Length];
						VkPipelineShaderStageCreateInfo* stagesPtr = stackalloc VkPipelineShaderStageCreateInfo[shaders.Modules.Length];
						for ( int i = 0; i < shaders.Modules.Length; i++ ) {
							var module = shaders.Modules[i];
							(var name, entryPointPins[i]) = CString.CreatePinned( module.EntryPoint );
							stagesPtr[i] = module.StageCreateInfo with { pName = name };
						}
						...
						foreach ( var i in entryPointPins ) {
							i.Free();
						}
					}
				}
