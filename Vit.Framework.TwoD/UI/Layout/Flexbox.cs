...
public class Flexbox<T> : FlowingLayoutContainer<T, FlexboxParams, Flexbox<T>.ChildArgs> where T : UIComponent {
	...
	protected override float PerformLayout ( LayoutContext context ) {
		...
		void finalizeLine ( ref SpanSlice<ChildLayout> line, Span<ChildArgs> args ) {
			if ( !isFirstLine ) crossPosition += gapSize.Cross;
			isFirstLine = false;
		...
		for ( int i = 0; i < children.Length; i++ ) {
			...
			if ( coversBothDirections && line.Length != 0 && lineSize.Flow + size.Flow > contentSize.Flow ) finalizeLine( ref line, children );
			...
			if ( coversBothDirections && lineSize.Flow > contentSize.Flow ) finalizeLine( ref line, children );
		}
