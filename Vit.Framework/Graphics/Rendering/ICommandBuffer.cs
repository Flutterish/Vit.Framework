...
public interface ICommandBuffer {
	...
	/// <summary>
	/// Draws instances using the index buffer.
	/// </summary>
	/// <param name="vertexCount">Amount of vertices to draw (<b>not</b> elements).</param>
	/// <param name="instanceCount">Amount of instances to draw.</param>
	/// <param name="offset">Offset from the start of the index buffer.</param>
	/// <param name="instanceOffset">The first instance to draw.</param>
	void DrawInstancesIndexed ( uint vertexCount, uint instanceCount, uint offset = 0, uint instanceOffset = 0 );
}
...
public abstract class BasicCommandBuffer<TRenderer, TFramebuffer, TTexture, TShaderSet> : ICommandBuffer
	...
	void ICommandBuffer.DrawIndexed ( uint vertexCount, uint offset ) {
		updateStateForDrawing();
		DrawIndexed( vertexCount, offset );
	}

	/// <inheritdoc cref="ICommandBuffer.DrawIndexed(uint, uint)"/>
	protected abstract void DrawIndexed ( uint vertexCount, uint offset );

	void ICommandBuffer.DrawInstancesIndexed ( uint vertexCount, uint instanceCount, uint offset, uint instanceOffset ) {
		updateStateForDrawing();
		DrawInstancesIndexed( vertexCount, instanceCount, offset, instanceOffset );
	}

	/// <inheritdoc cref="ICommandBuffer.DrawInstancesIndexed(uint, uint, uint, uint)"/>
	protected abstract void DrawInstancesIndexed ( uint vertexCount, uint instanceCount, uint offset, uint instanceOffset );
}
