...
public partial class Program : App {
	...
	public static void Main () {
		...
		using var host = new ConsoleHost( app );
		app.Run( host );
	...
	protected override void Initialize ( Host host ) {
		var a = host.CreateWindow( GraphicsApiType.Curses, this );
		a.Title = "Window A [Curses]";
		a.Initialized += _ => {
			var api = host.CreateGraphicsApi( GraphicsApiType.Curses, new[] { RenderingCapabilities.DrawToWindow } );
