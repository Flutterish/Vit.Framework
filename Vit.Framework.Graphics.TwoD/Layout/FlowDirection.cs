...
public static class FlowDirectionExtensions {
	...
	public static FlowPoint2<T> ToFlow<T> ( this FlowDirection flow, Point2<T> point, FlowSize2<T> size ) where T : INumber<T> {
		...
		if ( primary is FlowDirection.Left or FlowDirection.Down ) {
			aligned.Flow = size.Flow - aligned.Flow;
		}
		if ( secondary is FlowDirection.CrossLeft or FlowDirection.CrossDown ) {
			aligned.Cross = size.Cross - aligned.Cross;
		}
	...
	public static Point2<T> FromFlow<T> ( this FlowDirection flow, FlowPoint2<T> point, FlowSize2<T> size ) where T : INumber<T> {
		...
		if ( primary is FlowDirection.Left or FlowDirection.Down ) {
			point.Flow = size.Flow - point.Flow;
		}
		if ( secondary is FlowDirection.CrossLeft or FlowDirection.CrossDown ) {
			point.Cross = size.Cross - point.Cross;
		}
	...
	public static FlowSpacing<T> ToFlow<T> ( this FlowDirection flow, Spacing<T> spacing ) where T : INumber<T> {
		...
		if ( primary is FlowDirection.Left or FlowDirection.Down ) {
			(aligned.FlowStart, aligned.FlowEnd) = (aligned.FlowEnd, aligned.FlowStart);
		}
		if ( secondary is FlowDirection.CrossLeft or FlowDirection.CrossDown ) {
			(aligned.CrossStart, aligned.CrossEnd) = (aligned.CrossEnd, aligned.CrossStart);
		}
